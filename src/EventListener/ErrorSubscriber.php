...
class ErrorSubscriber implements SubscriberInterface, LoggerAwareInterface
{
    ...
    public function handleWebError(ApplicationErrorEvent $event): void
    {
        ...
        switch (true) {
            ...
            case $event->getError() instanceof RouteNotFoundException:
                ...
                $this->logger->error(sprintf('Route `%s` not found', $app->get('uri.route')), ['exception' => $event->getError()]);
                $this->prepareResponse($event);
    ...
    {
        $this->logger->error(sprintf('Uncaught Throwable of type %s caught.', \get_class($throwable)), ['exception' => $throwable]);
    }
    ...
    private function prepareResponse(ApplicationErrorEvent $event): void
    {
        ...
        switch (true) {
            case $app->input->getString('_format', 'json') === 'json':
            ...
            case $app->getResponse() instanceof JsonResponse:
                ...
                break;
            default:
                $response = new HtmlResponse($this->renderer->render('exception.twig', ['exception' => $event->getError()]));
